  ...
  <div class="app-recharge">
	 ...
	 <div class='title2'>
		 <ul>
			 ...
			 <li>
			 	 <span class='name' >合计平台充值</span>
			 	 <span class='number'>￥{{platformAmount}}</span>
			 </li>
			 <li>
			 	<span class='name'>合计跟单充值</span>
...
<script>
import { recharge } from '@/api/nested'
...
export default {
  ...
  data() {
    return {
       ...
       listLoading: true,
		...
		total:0,
		totalAmount:0,//合计充值
		platformAmount:0,//合计平台充值
		lotteryFollowAmount:0,//合计跟单充值
  ...
  methods: {
    fetchData() { 
      ...
      recharge({startDate:startDate,endDate:endDate}).then(response => {
	...
	dataList(list){
	      ...
	      for(let key in list){ 
			 ...
			 list[key].totalAmount=list[key].platformAmount+list[key].lotteryFollowAmount;
			 listArry.push(list[key]);
			 totalAmount=totalAmount+list[key].totalAmount;
			 platformAmount=platformAmount+list[key].platformAmount;
			 lotteryFollowAmount=lotteryFollowAmount+list[key].lotteryFollowAmount;
		  }
		 
		  this.totalAmount=totalAmount;
		  this.platformAmount=platformAmount;
		  this.lotteryFollowAmount=lotteryFollowAmount;
		  return listArry;
	},
	handleSerch(){
		this.fetchData();
